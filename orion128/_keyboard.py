...
The host keys are SDL key codes. PySDL2 is imported where a keyboard is
made, not when this module loads, so the rest of the package does without
it.
'''
...
class Keyboard:
    ...
    crossings that produce the shifted symbols. Each keyboard sets its own
    in its constructor.'''

    def __init__(self, keys: dict[int, Crossing], shift: Crossing,
                 shifted: dict[int, frozenset[Crossing]]) -> None:
        import sdl2
        self.__host_shift = frozenset({sdl2.SDLK_LSHIFT, sdl2.SDLK_RSHIFT})
        self.__keys = keys
        self.__shift = shift
    ...
    def crossings(self, held: set[int]) -> set[Crossing]:
        ...
        shift = bool(held & self.__host_shift)
        result: set[Crossing] = set()
        for key in held:
            if key in self.__host_shift:
                continue
...
class MS7007Keyboard(Keyboard):
    def __init__(self) -> None:
        import sdl2

        # The base map: letters (uppercase), digits, the unshifted
        # punctuation, and the control keys.
        keys: dict[int, Crossing] = {
        ...
        # The shifted symbols: host key to the crossings for its US-layout
        # shifted character. Every symbol has a key on this keyboard; a few
        # need the Orion shift as well (its own key is the shifted form).
        shifted: dict[int, frozenset[Crossing]] = {
        ...
        super().__init__(keys, (5, 9), shifted)


class RK86Keyboard(Keyboard):
    def __init__(self) -> None:
        import sdl2

        # A smaller keyboard: the regular RK-86 ASCII grid, letters and
        # digits in columns 2-7, the control keys in columns 0-1. Games that
        # scan the matrix directly, such as MANIC, rely on these exact
        # positions.
        keys: dict[int, Crossing] = {
        ...
        # Only these shifted symbols exist on the keyboard; the rest of the
        # PC's (!#$%&*()+=<>?) have no key here.
        shifted: dict[int, frozenset[Crossing]] = {
        ...
        super().__init__(keys, (4, 9), shifted)
